#!/usr/bin/env python3
"""
Bom Dia Mercado - gerador de relatorio.
Le o texto do boletim (corpo da issue), pede ao Claude os campos em JSON
e encaixa no template modelo-completo.html.
"""
import os
import sys
import json
import re
import base64
import urllib.request
from anthropic import Anthropic

MODELO = "modelo-completo.html"
SAIDA = "index.html"

# Bandeiras desenhadas em SVG: nao dependem de fonte nem de emoji.
BANDEIRAS = {
    "BR": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#009B3A"/>'
          '<path d="M14 2.5 25.5 10 14 17.5 2.5 10Z" fill="#FEDF00"/>'
          '<circle cx="14" cy="10" r="4.2" fill="#002776"/></svg>',
    "US": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#fff"/>'
          '<g fill="#B22234"><rect width="28" height="1.55" y="0"/>'
          '<rect width="28" height="1.55" y="3.1"/><rect width="28" height="1.55" y="6.2"/>'
          '<rect width="28" height="1.55" y="9.3"/><rect width="28" height="1.55" y="12.4"/>'
          '<rect width="28" height="1.55" y="15.5"/><rect width="28" height="1.55" y="18.4"/></g>'
          '<rect width="12" height="10.8" fill="#3C3B6E"/></svg>',
    "EU": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#003399"/>'
          '<g fill="#FFCC00"><circle cx="14" cy="5" r="1"/><circle cx="17.5" cy="6" r="1"/>'
          '<circle cx="19.5" cy="9" r="1"/><circle cx="18.5" cy="12.5" r="1"/>'
          '<circle cx="14" cy="14" r="1"/><circle cx="9.5" cy="12.5" r="1"/>'
          '<circle cx="8.5" cy="9" r="1"/><circle cx="10.5" cy="6" r="1"/></g></svg>',
    "GB": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#012169"/>'
          '<path d="M0 0 28 20M28 0 0 20" stroke="#fff" stroke-width="4"/>'
          '<path d="M0 0 28 20M28 0 0 20" stroke="#C8102E" stroke-width="2"/>'
          '<path d="M14 0v20M0 10h28" stroke="#fff" stroke-width="6"/>'
          '<path d="M14 0v20M0 10h28" stroke="#C8102E" stroke-width="3.5"/></svg>',
    "JP": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#fff"/>'
          '<circle cx="14" cy="10" r="5.5" fill="#BC002D"/></svg>',
    "CN": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#DE2910"/>'
          '<g fill="#FFDE00"><circle cx="6" cy="5.5" r="2.6"/><circle cx="11.5" cy="2.6" r="0.9"/>'
          '<circle cx="13.6" cy="5" r="0.9"/><circle cx="13.2" cy="8.2" r="0.9"/>'
          '<circle cx="10.8" cy="10" r="0.9"/></g></svg>',
    "DE": '<svg viewBox="0 0 28 20"><rect width="28" height="6.7" fill="#000"/>'
          '<rect width="28" height="6.7" y="6.7" fill="#DD0000"/>'
          '<rect width="28" height="6.6" y="13.4" fill="#FFCE00"/></svg>',
    "MX": '<svg viewBox="0 0 28 20"><rect width="9.4" height="20" fill="#006847"/>'
          '<rect width="9.2" height="20" x="9.4" fill="#fff"/>'
          '<rect width="9.4" height="20" x="18.6" fill="#CE1126"/></svg>',
    "AR": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#fff"/>'
          '<rect width="28" height="6.7" fill="#74ACDF"/>'
          '<rect width="28" height="6.7" y="13.3" fill="#74ACDF"/>'
          '<circle cx="14" cy="10" r="2.2" fill="#F6B40E"/></svg>',
    "XX": '<svg viewBox="0 0 28 20"><rect width="28" height="20" fill="#C9C4B8"/></svg>',
}


def bandeira_svg(codigo):
    return BANDEIRAS.get((codigo or "XX").upper(), BANDEIRAS["XX"])


def dados_da_issue():
    titulo = os.getenv("ISSUE_TITLE", "").strip()
    corpo = os.getenv("ISSUE_BODY", "").strip()
    if not corpo:
        print("ERRO: corpo da issue vazio")
        sys.exit(1)
    return titulo, corpo


def carregar_modelo():
    if not os.path.exists(MODELO):
        print(f"ERRO: {MODELO} nao encontrado na raiz do repositorio")
        sys.exit(1)
    with open(MODELO, encoding="utf-8") as f:
        return f.read()


PROMPT = """Voce recebe o texto bruto de um boletim de mercado brasileiro.
Extraia as informacoes e devolva APENAS um objeto JSON valido: sem markdown,
sem crases, sem nenhum texto antes ou depois.

Chaves obrigatorias:
- DATA_LONGA: data por extenso, ex "Sexta-feira, 18 de setembro de 2026"
- DATA_CURTA: ex "18/09/2026"
- DESCRIPTION: manchete resumida em uma linha, ate 150 caracteres
- MANCHETE_TITULO: titulo curto da manchete, em MAIUSCULAS
- MANCHETE_TEXTO: paragrafo de abertura, 2 a 4 frases
- DOLAR, DXY, BRENT, TNOTE: valores de fechamento (so o numero, ex "5,1543"; DXY ex "100,323")
- DOLAR_VAR, DXY_VAR, BRENT_VAR, TNOTE_VAR: variacao, ex "+0,14%" ou "—"
- DOLAR_COR, DXY_COR, BRENT_COR, TNOTE_COR: "pos" se subiu, "neg" se caiu, "neu" se estavel
- AGENDA: lista de objetos com as chaves "pais" (codigo de duas letras do pais:
  BR, US, EU, GB, JP, CN, DE, MX, AR; use XX se nao se aplicar),
  "hora" (ex "8h00" ou "no dia"), "titulo" e "detalhe" (detalhe pode ser "")
- TEXTO_JUROS, TEXTO_DOLAR, TEXTO_BOLSA, TEXTO_ELEICOES, TEXTO_PETROLEO,
  TEXTO_PETROBRAS: o texto corrido de cada secao, reescrito de forma fluida,
  um paragrafo cada. Se nao houver informacao para a secao, use
  "Nenhuma noticia relevante."

Regras:
- Nao invente numeros que nao estao no texto; preserve os valores exatamente
  como aparecem no boletim.
- Nao use markdown dentro dos textos (nada de ** ou #).

TEXTO DO BOLETIM:
---
{corpo}
---"""


INSTRUCAO_FOTO = """
ATENCAO: junto com o texto veio uma FOTO do painel de cotacoes do Broadcast.
Para os campos do painel, use SEMPRE os numeros da foto, e nao os do texto.

A foto pode vir em dois formatos:
(a) LISTA: uma linha por ativo, com as colunas ATIVO, ULTIMO e VARIACAO.
(b) CARDS: dois cards por linha; dentro do card vem o codigo do ativo, a
    descricao, o valor em numero grande e, logo abaixo, a variacao com um
    horario ao lado (ex "05:44"). NAO confunda o horario com a variacao,
    nem com o valor.

De onde vem cada campo:
- DOLAR = USDBRL, 4 casas decimais, ex "5,1679"
- DXY = DXY, 3 casas decimais, ex "101,223"
- BRENT = IBRENT, 2 casas decimais, ex "105,52"
- TNOTE = US10YY, arredondado para 2 casas decimais, ex "5,15"
  (cuidado: US2YY e US30YY sao outros vencimentos; use so o de 10 anos)

Variacao e cor:
- Use a variacao do proprio ativo, e nao a de outro card.
- SINAL: na foto a variacao pode aparecer SEM o sinal de mais. Nesse caso,
  use a COR do numero: verde = alta, vermelho = queda. Ex: "0,13%" em verde
  vira VAR "+0,13%" e COR "pos"; "0,75%" em vermelho vira "-0,75%" e "neg".
  Se a variacao ja vier com sinal, respeite o sinal.
- *_COR = "pos" se a variacao for positiva, "neg" se negativa, "neu" se for
  zero (ex "-0,00%") ou se no lugar da variacao aparecer so uma data.
- Se no lugar da variacao houver uma data (ex "23/09"), significa que o
  mercado esta fechado: use VAR "—" e COR "neu".
- Se algum ativo nao aparecer na foto, use o valor do texto; se nao houver
  em nenhum dos dois, use "—" e cor "neu".

Preserve o formato brasileiro: virgula decimal e ponto de milhar.
"""

PADRAO_IMAGEM = re.compile(
    r'(?:!\[[^\]]*\]\((https?://[^)\s]+)\))|(?:<img[^>]+src="(https?://[^"]+)"[^>]*>)',
    re.IGNORECASE,
)


def separar_imagens(corpo):
    """Retorna (texto sem as imagens, lista de URLs das imagens)."""
    urls = [a or b for a, b in PADRAO_IMAGEM.findall(corpo)]
    texto = PADRAO_IMAGEM.sub("", corpo).strip()
    return texto, urls


def tipo_imagem(dados):
    if dados.startswith(b"\x89PNG"):
        return "image/png"
    if dados.startswith(b"\xff\xd8"):
        return "image/jpeg"
    if dados[:4] == b"RIFF" and dados[8:12] == b"WEBP":
        return "image/webp"
    if dados[:6] in (b"GIF87a", b"GIF89a"):
        return "image/gif"
    return None


def baixar_imagem(url):
    """Baixa a foto anexada na issue. Usa o token do GitHub se houver
    (necessario em repositorio privado). Retorna bloco de imagem ou None."""
    headers = {"User-Agent": "bom-dia-mercado"}
    token = os.getenv("GITHUB_TOKEN")
    if token:
        headers["Authorization"] = f"token {token}"
    try:
        req = urllib.request.Request(url, headers=headers)
        with urllib.request.urlopen(req, timeout=60) as r:
            dados = r.read()
    except Exception as e:
        print(f"AVISO: nao consegui baixar a imagem ({e}). Sigo so com o texto.")
        return None
    tipo = tipo_imagem(dados)
    if not tipo:
        print("AVISO: o anexo nao parece ser uma imagem. Sigo so com o texto.")
        return None
    if len(dados) > 5 * 1024 * 1024:
        print("AVISO: imagem maior que 5 MB. Sigo so com o texto.")
        return None
    print(f"Imagem do painel carregada ({tipo}, {len(dados)} bytes).")
    return {
        "type": "image",
        "source": {"type": "base64", "media_type": tipo,
                   "data": base64.b64encode(dados).decode()},
    }


def pedir_campos(corpo):
    texto, urls = separar_imagens(corpo)
    imagem = baixar_imagem(urls[0]) if urls else None

    prompt = PROMPT.format(corpo=texto)
    conteudo = []
    if imagem:
        conteudo.append(imagem)
        prompt = INSTRUCAO_FOTO + "\n" + prompt
    conteudo.append({"type": "text", "text": prompt})

    client = Anthropic()
    resp = client.messages.create(
        model="claude-sonnet-4-6",
        max_tokens=8000,
        messages=[{"role": "user", "content": conteudo}],
    )
    bruto = "".join(b.text for b in resp.content if b.type == "text").strip()
    bruto = re.sub(r"^```(?:json)?|```$", "", bruto, flags=re.MULTILINE).strip()
    try:
        return json.loads(bruto)
    except json.JSONDecodeError as e:
        print(f"ERRO ao interpretar o JSON devolvido pelo Claude: {e}")
        print("--- inicio da resposta ---")
        print(bruto[:800])
        sys.exit(1)


def montar_agenda(itens):
    linhas = []
    for it in itens or []:
        detalhe = (it.get("detalhe") or "").strip()
        bloco = f"\n          <span>{detalhe}</span>" if detalhe else ""
        linhas.append(
            "      <li>\n"
            f'        <span class="bandeira">{bandeira_svg(it.get("pais"))}</span>\n'
            f'        <span class="hora">{it.get("hora", "")}</span>\n'
            '        <span class="desc">\n'
            f'          <strong>{it.get("titulo", "")}</strong>{bloco}\n'
            "        </span>\n"
            "      </li>"
        )
    return "\n".join(linhas)


def preencher(modelo, campos):
    html = modelo

    agenda_html = montar_agenda(campos.get("AGENDA"))
    html = re.sub(
        r'(<ul class="agenda-lista">).*?(</ul>)',
        lambda m: m.group(1) + "\n" + agenda_html + "\n    " + m.group(2),
        html,
        flags=re.DOTALL,
    )

    campos.setdefault("DATA_CURTA_FOOTER", campos.get("DATA_CURTA", ""))

    for chave, valor in campos.items():
        if chave == "AGENDA":
            continue
        html = html.replace("{{" + chave + "}}", str(valor))

    html = re.sub(r"\{\{[A-Z_]+\}\}", "", html)
    return html


def main():
    print("Iniciando geracao do relatorio...")
    titulo, corpo = dados_da_issue()
    print(f"Titulo: {titulo}")
    print(f"Corpo: {len(corpo)} caracteres")

    modelo = carregar_modelo()
    print("Modelo carregado.")

    campos = pedir_campos(corpo)
    print(f"Campos recebidos: {len(campos)}")

    html = preencher(modelo, campos)

    with open(SAIDA, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"{SAIDA} salvo ({len(html)} bytes)")
    print("Relatorio gerado com sucesso.")


if __name__ == "__main__":
    main()
